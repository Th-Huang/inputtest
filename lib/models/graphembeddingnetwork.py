...
from lib.models.segment import unsorted_segment_sum
...
class GraphPropLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self._node_hidden_sizes = node_hidden_sizes[:] + [node_state_dim*2]
    ...
    def build_model(self):
        layer = []
        layer.append(nn.Linear(256*2, self._edge_hidden_sizes[0]*2))
        for i in range(1, len(self._edge_hidden_sizes)):
            layer.append(nn.ReLU())
            layer.append(nn.Linear(self._edge_hidden_sizes[i - 1]*2, self._edge_hidden_sizes[i]))
        ...
        if self._use_reverse_direction:
            if self._reverse_dir_param_different:
                layer = []
                layer.append(nn.Linear(self._node_state_dim*4 + self._edge_state_dim, self._edge_hidden_sizes[0]))
                for i in range(1, len(self._edge_hidden_sizes)):
                    layer.append(nn.ReLU())
                    layer.append(nn.Linear(self._edge_hidden_sizes[i - 1], self._edge_hidden_sizes[i]))
        ...
        if self._node_update_type == 'gru':
            if self._prop_type == 'embedding':
                ...
                self.GRU = torch.nn.GRU(384, 128)
        else:
            layer = []
            if self._prop_type == 'embedding':
                layer.append(nn.Linear(self._node_state_dim * 3, self._node_hidden_sizes[0]))
            elif self._prop_type == 'matching':
                layer.append(nn.Linear(self._node_state_dim * 6, self._node_hidden_sizes[0]*2))
            for i in range(1, len(self._node_hidden_sizes)):
                layer.append(nn.ReLU())
                layer.append(nn.Linear(self._node_hidden_sizes[i - 1]*2, self._node_hidden_sizes[i]))
    ...
    def _compute_aggregated_messages(
        ...
        aggregated_messages, ef = graph_prop_once(
            node_states,
            from_idx,
            to_idx,
            self._message_net,
        ...
        # optionally compute message vectors in the reverse direction
        if self._use_reverse_direction:
            reverse_aggregated_messages, ef = graph_prop_once(
                node_states,
                to_idx,
                from_idx,
...
class GraphAggregator(nn.Module):
    ...
    def build_model(self):
        ...
        layer = []
        layer.append(nn.Linear(128, node_hidden_sizes[0]))
    ...
    def forward(self,
        ...
        for layer in self._prop_layers:
            ...
            states = self._apply_layer(
                ...
                n_graphs,
                edge_features)
            node_states = states[0]
            edge_features = states[1]
